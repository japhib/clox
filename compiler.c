...
static void errorAt(Token* token, const char* message) {
    // set panicMode to true to swallow errors until next synchronization point (statment boundary)
...
static void consume(TokenType type, const char* errMessage) {
    if (parser.current.type == type) {
        advance();
        return;
    }

    errorAtCurrent(errMessage);
}
